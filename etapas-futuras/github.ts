/**
 * Camada mais baixa: fala HTTP com a API REST do GitHub.
 *
 * Nada aqui sabe o que é uma "nota" ou o que é o Obsidian — isso fica em
 * `vault.ts`. Aqui só existem repositórios, branches, blobs e commits.
 */
...
async function request(
  ...
  const res = await fetch(`${GITHUB_API}${path}`, {
    ...
    // Sempre buscar do GitHub: o cache é nosso (vault.ts), não do fetch.
    cache: "no-store",
  });
...
export type TreeEntry = {
  ...
  path: string;
  /** Hash do conteúdo — usado para baixar o blob e para detectar mudanças */
  sha: string;
  size: number;
};
